%
%  regionHistory_EQUALOBJECTLENGTH.m
%  SMI
%
...
function [activity] = regionHistory_EQUALOBJECTLENGTH(filename, region, depth, maxEpoch)
    ...
    if(any(numOutputsPrObject(1)*ones(1,historyDimensions.numObjects) ~= numOutputsPrObject')),
        error('All objects must have same number of outputs.'); % Check that indeed all objects have same output
    ...
    activity = reshape(buffer, [numOutputsPrObject(1) historyDimensions.numObjects maxEpoch y_dimension x_dimension]);
